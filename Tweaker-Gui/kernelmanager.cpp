...
void KernelManager::onInstallKernelToDevice()
{
    ...
    for (const QString &device : blockDevices) {
        ...
        if (!device.contains(QRegExp("[0-9]$")) || device.startsWith("mmcblk")) {
...
void KernelManager::onCopyCurrentKernel()
{
    ...
    // Update path edit based on radio selection
    auto updatePath = [&]() {
        if (toTweakerDirRadio->isChecked()) {
            pathEdit->setText(m_kernelDirectoryEdit->text());
        } else {
            pathEdit->setText("/mnt");
        }
    };
    
    connect(toTweakerDirRadio, &QRadioButton::toggled, updatePath);
    connect(toMountedDeviceRadio, &QRadioButton::toggled, updatePath);
